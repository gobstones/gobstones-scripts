/* eslint-disable */
/**
 * This module provides tooling to be used
 * on the `package-scripts.js` file, so it helps on writing
 * `nps` commands.
 *
 * @module Tools
...
const internal_api = require('../api/internal_api');
...
const config = internal_api.config;
 ...
 * @param options "tsc" requires options with the following signature:
 * ```
    {
...
function tsc(options) {
    ...
    return (
        `tsc  --project ${config.tsConfigFile} ${options.file}`
    );
}
...
function webpack(options) {
    ...
    return `webpack --config ${config.files.webpack}`;
}
...
function rollup(options) {
    options = options || {};
    return `rollup --config ${config.files.rollup}` + (options.watch ? ` --watch ${options.watch}` : '');
}
   ...
   function jest(options) {
    ...
    return ifNotWindows(
        'if grep -l "\\.only" ./test/**/*.test.ts; ' +
            'then grep -l "\\.only" ./test/**/*.test.ts | xargs ' + jestStringBase+ '; ' +
            'else ' + jestStringBase +
            additionalArgs +
            '; ' +
            'fi',
...
function typedoc(options) {
    options = options || {};
    return `typedoc --options ${config.files.typedoc} --tsconfig ${config.tsConfigFile}` + (options.watch ? ` --watch ${options.watch}` : '');
}
...
function prettier(options) {
    ...
    return `prettier --config --write ${options.files}`;
}
